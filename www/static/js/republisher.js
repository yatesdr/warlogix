    ...
    fetch('/htmx/tags/' + encodeURIComponent(plc) + '/' + encodeURIComponent(tagName))
        ...
        .then(function(data) {
            ...
            if (data.member_types) {
                ...
                // Mark that we tried — no member types for this PLC type
            ...
            if (!hasValue) {
                ...
                if (isStruct && !tagItem.classList.contains('has-children')) {
                    ...
                    if (tagRow && !tagRow.querySelector('.published-child-count')) {
                        var pubChildren = {};
                        ...
                        // Insert after tag-name span
                        var nameEl = tagRow.querySelector('.tag-name');
                        if (nameEl && nameEl.nextSibling) {
                            tagRow.insertBefore(countSpan, nameEl.nextSibling);
                        } else {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        if (Array.isArray(value) && valueType && valueType.endsWith('[]') &&
            ...
            html += '<div class="tag-child-row' + (ignored ? ' ignored' : '') + (published ? ' published' : '') + '" ' +
                ...
                indicators +
                '<span class="child-key">' + WarLink.escapeHtml(displayKey) + '</span>' +
...
function updateDetailsPanel(info) {
    ...
    try {
        ...
        if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed)) {
            ...
            valueHtml = '<div class="value-simple mono">' + WarLink.escapeHtml(String(parsed)) + '</div>';
        }
    } catch (e) {
        valueHtml = '<div class="value-simple mono">' + WarLink.escapeHtml(info.json || '-') + '</div>';
    }
    ...
    details.innerHTML =
        ...
        '<tr><th>PLC</th><td>' + WarLink.escapeHtml(info.plc) + '</td></tr>' +
        '<tr><th>Name</th><td class="mono">' + WarLink.escapeHtml(displayName) + '</td></tr>' +
        (info.alias ? '<tr><th>Alias</th><td>' + WarLink.escapeHtml(info.alias) + '</td></tr>' : '') +
        '<tr><th>Type</th><td class="mono">' + WarLink.escapeHtml(info.type || '-') + '</td></tr>' +
        (info.lastPoll ? '<tr><th>Last Read</th><td>' + WarLink.escapeHtml(info.lastPoll) + '</td></tr>' : '') +
        (info.lastChanged ? '<tr><th>Last Changed</th><td>' + WarLink.escapeHtml(info.lastChanged) + '</td></tr>' : '') +
        (info.hasIgnores && !info.path ? '<tr><th>Ignores</th><td>' + info.ignoreCount + ' members</td></tr>' : '') +
        '</table>' +
        tagManageHtml +
        '<div style="display:flex;align-items:center;gap:0.5rem;margin-top:1rem"><h4 style="margin:0">Value</h4>' +
        '<button class="btn btn-sm" onclick="forceReadTag(\'' + escapeAttr(info.plc) + '\',\'' + escapeAttr(info.tagName) + '\'' +
        (info.path ? ',\'' + escapeAttr(info.path) + '\'' : '') +
        ')" title="Force read from PLC">Re-read</button></div>' +
        valueHtml +
        writeHtml +
        '</div>';
...
function forceReadTag(plc, tagName, path) {
    fetch('/htmx/tags/' + encodeURIComponent(plc) + '/' + encodeURIComponent(tagName))
        .then(function(resp) {
            if (!resp.ok) return resp.text().then(function(msg) { throw new Error(msg); });
            return resp.json();
        ...
        .then(function(data) {
            // Update the tag item's cached data
            var plcGroup = document.querySelector('.plc-group[data-plc="' + plc + '"]');
            if (!plcGroup) return;
            var tagItem = plcGroup.querySelector('.tag-item[data-name="' + tagName + '"]');
            if (!tagItem) return;
            ...
            var resolvedValue = data.value;
            if (path) {
                var parts = path.split('.');
                for (var i = 0; i < parts.length && resolvedValue !== undefined; i++) {
            ...
            // Update selectedInfo and refresh the details panel
            if (selectedInfo && selectedInfo.plc === plc && selectedInfo.tagName === tagName) {
                selectedInfo.json = path ? JSON.stringify(resolvedValue) : jsonValue;
                if (data.type && !path) selectedInfo.type = data.type;
                updateDetailsPanel(selectedInfo);
            }
        })
        .catch(function(err) {
            WarLink.toast('Read failed: ' + err.message, 'error');
        });
}
...
function updatePublishedChildren(plc, tagName, path, published, writable) {
    ...
    if (published) {
        ...
        if (publishedChildren[path]) {
            publishedChildren[path] = {enabled: false, writable: false};
        }
    }
    ...
    if (countEl) {
        ...
        for (var k in publishedChildren) {
            if (publishedChildren[k] && publishedChildren[k].enabled) count++;
        }
        countEl.textContent = '(' + count + ' published children)';
        countEl.dataset.publishedCount = count;
    }
            ...
            if (Array.isArray(value)) {
                ...
                if (valueType && valueType.endsWith('[]') && value.length > 0 && typeof value[0] !== 'object') {
                    html += '<div class="tree-node">' +
                        '<span class="tree-key">' + WarLink.escapeHtml(key) + '</span>' +
...
function formatPrimitive(value) {
    ...
    if (typeof value === 'string') {
        return '<span class="text-string">"' + WarLink.escapeHtml(value) + '"</span>';
    }
    return WarLink.escapeHtml(String(value));
}
...
function buildPLCPickerItems() {
    ...
    groups.forEach(function(group) {
        ...
        item.innerHTML = '<span class="status-dot ' + statusClass + '"></span>' +
            '<span class="plc-picker-item-name">' + WarLink.escapeHtml(plcName) + '</span>' +
            '<span class="plc-picker-item-detail">' + WarLink.escapeHtml(address) + '</span>' +
            '<span class="plc-picker-item-count">' + WarLink.escapeHtml(tagCount) + '</span>';
...
function updatePLCPickerDisplay() {
    ...
    display.innerHTML = '<span class="status-dot ' + statusClass + '"></span>' +
        '<span class="plc-picker-display-text">' + WarLink.escapeHtml(selectedPLC) + '</span>' +
    ...
    if (selectedInfo && selectedInfo.plc && selectedInfo.tagName) {
        ...
        ignoreList = Array.isArray(parsedIgnore) ? parsedIgnore : [];
            ...
            if (selectedInfo.path) {
                // Child was selected - only restore child selection if parent is already expanded
                var plcForKey = tagItem.closest('.plc-group');
                var tagExpandKey = (plcForKey ? plcForKey.dataset.plc : '') + '::' + selectedInfo.tagName;
                if (expandedTags[tagExpandKey]) {
...
function showAddTagModal(plcName) {
    ...
    fetch('/htmx/plcs/' + encodeURIComponent(plcName) + '/types')
        ...
        .catch(function(err) {
            WarLink.toast('Error loading PLC type info: ' + err.message, 'error');
        });
}
...
function buildTagModalFields(typeInfo, existing) {
    ...
    for (var i = 0; i < types.length; i++) {
        ...
        typeOptionsHtml += '<option value="' + escapeAttr(types[i]) + '"' + selected + '>' + WarLink.escapeHtml(types[i]) + '</option>';
    }
    ...
    if (isAddressBased) {
        ...
        html += '<div class="form-group"><label>' + WarLink.escapeHtml(addressLabel) + '</label><input type="text" class="form-control" id="tag-modal-name" value="' + nameValue + '" required placeholder="e.g. ' + (addressLabel === 'DB.Offset' ? 'DB1.DBD0' : 'DM100') + '"></div>';
    } else {
        // Tag-based: Tag Name, Type, Alias
        html += '<div class="form-group"><label>' + WarLink.escapeHtml(addressLabel) + '</label><input type="text" class="form-control" id="tag-modal-name" value="' + nameValue + '" required></div>';
        html += '<div class="form-group"><label>Data Type</label><select class="form-control" id="tag-modal-type">' + typeOptionsHtml + '</select></div>';
        html += '<div class="form-group"><label>Alias</label><input type="text" class="form-control" id="tag-modal-alias" value="' + aliasValue + '" placeholder="Optional display name"></div>';
    }
...
function submitTagModal(event) {
    ...
    if (!tagName) {
        WarLink.toast('Tag name/address is required', 'error');
        return;
    }
    ...
    function doSave() {
        ...
        .catch(function(err) {
            WarLink.toast('Error saving tag: ' + err.message, 'error');
        });
    }
    ...
    if (deleteOld) {
        ...
        .catch(function(err) {
            WarLink.toast('Error removing old tag: ' + err.message, 'error');
        });
    } else {
        doSave();
    }
...
function deleteTag(plcName, tagName) {
    ...
    .catch(function(err) {
        WarLink.toast('Error deleting tag: ' + err.message, 'error');
    });
}
