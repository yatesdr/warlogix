...
func (p *PTYTty) Start() error {
	...
	termios, err := unix.IoctlGetTermios(int(p.file.Fd()), unix.TCGETS)
	if err != nil {
		return err
	}
	...
	if err := unix.IoctlSetTermios(int(p.file.Fd()), unix.TCSETS, &rawTermios); err != nil {
		return err
	}
...
func (p *PTYTty) Stop() error {
	...
	if p.saved != nil {
		return unix.IoctlSetTermios(int(p.file.Fd()), unix.TCSETS, *p.saved)
	}
	return nil
}
